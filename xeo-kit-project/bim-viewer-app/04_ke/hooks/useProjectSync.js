...
export const useProjectSync = (file) => {
  ...
  const updateStretch = (id, axis, stretchValue, newPosition) => {
    if (!id) return;
    ...
    setProjectState(prev => ({
      ...
      furniture: prev.furniture.map(f => {
        ...
        const updatedStretch = [...(f.stretch || [1, 1, 1])];
        updatedStretch[axis] = stretchValue;
        return { ...f, stretch: updatedStretch, position: newPosition || f.position };
      }),
    }));
  };
