...
export const useStretchEditor = (viewerRef, canvasRef, onPersist) => {
  ...
  useEffect(() => {
    ...
    const onCanvasMouseMove = (e) => {
      ...
      if (now - lastHoverPickRef.current < 80) return;
  ...
  const aabbRef = useRef(null);
  aabbRef.current = aabb;
  ...
  useEffect(() => {
    ...
    const recompute = () => {
      const currentAabb = aabbRef.current;
      if (!currentAabb) {
        setScreenGeometry(null);
        return;
      }
      ...
      const corners = boxCorners(currentAabb).map(p => worldToScreen(p, camera, w, h));
