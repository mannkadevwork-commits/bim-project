  ...
  const configureTransformHandles = (mode) => {
    ...
    stretchHandlesRef.current.forEach(mesh => {
      ...
      if (meta.transformMode === 'stretch' && baseMode === 'stretch') {
        // Resize starts with the six primary face grips only. The face click
        // reveals its adjacent 2-axis/3-axis grips.
  ...
  useEffect(() => {
    ...
    const onCanvasMouseDown = (e) => {
      ...
      if (mode === 'stretch' && (type === 'face' || type === 'edge' || type === 'corner')) {
        if (type === 'face') {
          const faceKey = `${axes[0].axis}:${axes[0].dir}`;
          activeResizeFaceKeyRef.current = faceKey;
    ...
    const onDocMouseUp = () => {
      ...
      const expandedFace = activeResizeFaceKeyRef.current;
      setTimeout(() => {
        configureTransformHandles(transformModeRef.current);
        if (expandedFace && transformModeRef.current === 'stretch') {
          revealGroupForFace(expandedFace, stretchFaceAdjacencyRef, revealedFaceKeyRef, revealedHandlesRef, stretchAnimFramesRef);
        }
      }, 0);
    };
