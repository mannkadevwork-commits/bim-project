...
const BIMViewer = ({ activeProject, onDelete, onAdd }) => {
  ...
  const pendingSelectionPosRef = useRef({ x: 0, y: 0 });
  ...
  const handlePointerDown = (e) => {
    ...
    pendingSelectionPosRef.current = { x: e.clientX, y: e.clientY };
  };

  // Anchor the contextual toolbar only when selection actually changes.
  // This prevents camera drags or clicks on an already-selected element from
  // making the toolbar jump around.
  useEffect(() => {
    if (engineState.selectedAssetId || engineState.selectedObject) {
      setLastClickPos(pendingSelectionPosRef.current);
    }
  }, [engineState.selectedAssetId, engineState.selectedObject?.id]);
