...
export const useBIMEngine = (activeProject, projectStateRef, projectState, onAssetPlaced, setIsRightPanelOpen, setRightTab) => {
  ...
  useEffect(() => {
    ...
    // Snapshot selected IDs into a static array before clearing.
    // viewer.scene.selectedObjectIds is a live getter — passing it directly
    // to setObjectsSelected(..., false) returns an empty array mid-call
    // and the deselection silently becomes a no-op, leaving the viewer
    // stuck in a selected state where pickedNothing can never clear it.
    const clearSelection = () => {
      const ids = [...viewer.scene.selectedObjectIds];
      if (ids.length) viewer.scene.setObjectsSelected(ids, false);
    };
    ...
    const onCanvasMouseDown = (e) => {
      ...
      const baseMode = mode?.startsWith('stretch') ? 'stretch' : mode;
      if (meta.transformMode !== baseMode) return;
      ...
      if (baseMode === 'stretch' && (type === 'face' || type === 'corner2d')) {
  ...
  return {
    ...
    actions: {
      ...
      setTransformMode: (mode) => {
        const validModes = ['move', 'rotate', 'stretch', 'stretch-1d', 'stretch-2d', 'stretch-3d'];
