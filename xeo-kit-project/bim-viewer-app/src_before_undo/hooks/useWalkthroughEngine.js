...
import { LightingDirector } from './walkthrough/lightingDirector.js';
...
class WalkRuntime {
  constructor({ canvas, onState }) {
    ...
    this.lightingDirector = new LightingDirector({
      ...
      onChange: (state) => {
        this._updateLightingControl(state);
        this.onState?.({ type: 'lighting', ...state });
      },
    });
    this._createLightingControl();
    this._updateLightingControl(this.lightingDirector.getState());
  ...
  _createLightingControl() {
    if (this.lightingControl) return;
    ...
    root.className = 'hci-walkthrough-lighting-control';
    root.setAttribute('aria-label', 'Lighting mode');
    root.style.cssText = [
      'position:absolute',
      'right:24px',
      'bottom:78px',
      ...
      'z-index:30',
      'font:600 11px/1 Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',
      'pointer-events:auto',
      'user-select:none',
    ].join(';');
    ...
    title.style.cssText = 'padding:0 5px 0 7px;color:rgba(255,255,255,.64);letter-spacing:.02em;';
    ...
    this.lightingControl = { root, buttons };
  }

  _updateLightingControl(state) {
    const control = this.lightingControl;
    if (!control) return;
    const active = state?.targetMode || state?.mode || 'day';
    control.buttons.forEach((button, mode) => {
      const selected = mode === active;
      button.style.background = selected ? 'rgba(255,170,96,.18)' : 'transparent';
      button.style.color = selected ? '#fff' : 'rgba(255,255,255,.68)';
  ...
  _bind() {
    this.onKeyDown = (e) => {
      ...
      // L cycles the presentation lighting independently of overview/walk state.
      if (key === 'l') {
        e.preventDefault();
        this.cycleLighting();
        return;
  ...
  async load(jobId, baseUrl) {
    ...
    this.lightingDirector.attachModel(this.model);
    this.onState?.({ type: 'lighting', ...this.lightingDirector.getState() });
    ...
    this.lightingDirector.setRoomAnchors(this.walkAreas);
...
if (name === 'top') {
  ...
  _tick = () => {
    ...
    this._lastDt = dt;
    this.lightingDirector?.update(dt);
    if (this.viewMode === 'walk') {
  ...
  setLightingMode(mode) {
    return this.lightingDirector?.setMode(mode);
  }

  cycleLighting() {
    return this.lightingDirector?.cycle();
  }
  ...
  dispose() {
    ...
    if (this.lightingControl?.root) this.lightingControl.root.remove();
    this.lightingControl = null;
...
export function useWalkthroughEngine({ containerRef, jobId }) {
  ...
  useEffect(() => {
    ...
    const runtime = new WalkRuntime({
      ...
      onState: (event) => {
        ...
        if (event.type === 'lighting') setState((prev) => ({ ...prev, lightingMode: event.targetMode || event.mode || 'day' }));
  ...
  const setLightingMode = useCallback((value) => runtimeRef.current?.setLightingMode(value), []);
  const cycleLighting = useCallback(() => runtimeRef.current?.cycleLighting(), []);
  ...
  return { ...state, travelTo, switchRoom, navigateToHotspot, stopTravel, recoverToSafeSpot, setHeightOffset, setSensitivity, setWalkMode, setLookLocked, setViewMode, setAutoRotate, setViewPreset, zoom, fitView, setFov, setLightingMode, cycleLighting };
}