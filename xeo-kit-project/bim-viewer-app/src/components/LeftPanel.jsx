            ...
            {availableAssets.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                    {availableAssets.map(asset => (
                        <button key={asset.id} onClick={() => { setPlacementMode(asset); resetSelection(); }} className={`flex flex-col items-center justify-center p-4 border rounded-xl transition-all group ${placementMode?.id === asset.id ? 'bg-indigo-50 border-indigo-500 dark:bg-indigo-900/30 dark:border-indigo-400 shadow-sm' : 'bg-slate-50 hover:bg-white dark:bg-slate-800/50 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700'}`}>
