    ...
    >
      <div className={`relative ${placement.above ? 'origin-bottom-left' : 'origin-top-left'}`}>
        ...
        {mode === 'stretch' && !isNative && onResizeSubmodeChange && (
          <div className="mt-2 rounded-xl border border-amber-500/20 bg-[#0b1322]/96 backdrop-blur-2xl px-2 py-1.5 shadow-[0_14px_35px_rgba(0,0,0,0.35)] pointer-events-auto"
            onPointerDown={(e) => e.stopPropagation()}
            onMouseDown={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-1 mb-1.5">
              <span className="text-[8px] font-bold uppercase tracking-[0.14em] text-slate-500">Resize by</span>
              <span className="text-[7px] text-slate-600">choose handles</span>
            </div>
            <div className="grid grid-cols-3 gap-1">
              {[
                { id: 'face', label: 'Face', hint: '1 axis', Icon: Square },
                { id: 'edge', label: 'Edge', hint: '2 axes', Icon: Diamond },
                { id: 'corner', label: 'Corner', hint: '3 axes', Icon: Box },
                ...
                return (
                  <button
                    ...
                    className={`min-w-0 rounded-lg border px-2 py-1.5 text-center transition ${active ? 'border-amber-400/55 bg-amber-500/12 text-amber-200 shadow-[0_0_0_1px_rgba(251,191,36,0.08)]' : 'border-slate-700/70 bg-slate-900/30 text-slate-500 hover:text-slate-200 hover:border-slate-500'}`}
                    title={`${label}: ${hint}`}
                  >
                    ...
                    <div className="text-[7px] mt-0.5 opacity-60">{hint}</div>
                  </button>
                );
              })}
            </div>
