...
export const isolateAndMakeMoveable = async (ctx, entityId, onAdoptCallback, updateStructuralEdit) => {
  ...
  try {
    ...
    if (isolatedModel && viewerRef.current?.scene) {
      const selectedIds = [...viewerRef.current.scene.selectedObjectIds];
      for (const id of selectedIds) {
        const object = viewerRef.current.scene.objects[id];
        if (object) object.selected = false;
      }
      for (const model of Object.values(viewerRef.current.scene.models || {})) {
