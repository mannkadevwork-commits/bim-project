...
export const useProjectSync = (file) => {
  ...
  const updateAsset = (viewerRef, selectedAssetId, axis, value, isRotation = false) => {
    ...
    setProjectState(prev => ({ 
      ...prev, 
      furniture: prev.furniture.map(f => f.instanceId === selectedAssetId ? { ...f, position: updatedPos || f.position, rotation: updatedRot || f.rotation } : f) 
    }));
  };
  ...
  const spawnAsset = (asset, coordinates, loadIFCAssetIntoScene) => {
    ...
    const fullAssetUrl = `${API_BASE_URL}${asset.url}`;

    setProjectState(prev => ({ 
      ...prev, 
      furniture: [...prev.furniture, { id: asset.id, instanceId: uniqueId, name: asset.name, src: fullAssetUrl, position: coordinates, rotation: [0, 0, 0] }] 
    }));
